...
%% Stabilize the transfer function from vel to tilt (POST INTEGRATOR)
% HOMEMADE
...
%% Estimate transfer function for base system using LINEARIZE
% Transfer function tilt_ref to tilt_angle
% Velcity reference to tilt angle
load_system(model);
open_system(model);
% define points in model
...
w_c = phaseBalance_equation(deg2rad(phase_margin), alpha, Ni, Gtv_post);
...
% Find the Kp
open_loop_tf = Cpi * Cd * Gtv_post;
[mag, ~, ~] = bode(open_loop_tf, w_c); % 'bode' returns magnitude in absolute units (not dB), so no need to convert
Kp = 1 / squeeze(mag);
G_ol = Kp*Cpi*Cd*Gtv_post;
...
G_cl_fdb = (Kp*Cpi*Gtv_post) / (1 + Kp*Cpi*Gtv_post*Cd);
