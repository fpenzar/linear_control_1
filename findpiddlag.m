...
% 
function [wc, Kp, taui, taud, ok] = findpiddlag(G1, gm, Ni, al, beta, w)
    % find phase to look for
    % phii = -90 + atan(Ni) * 180 / pi;
    phiM = asin((1-al)/(1+al)) * 180/pi;
    phii = atan2(Ni*(1-beta), 1+beta*pow2(Ni)) * 180/pi;
    PG = -180 + gm - 2*phiM - phii;
    ...
    for j = 1:n
        ...
        Ci = tf([ti(j) 1],[ti(j) 1/beta]);
        [mj,pj] = bode(G1*Cd*Cd*Ci,wcs(j));
        kp(j) = 1/mj;
        Gol(j) = kp(j)*Cd*Cd*G1*Ci;
    end
